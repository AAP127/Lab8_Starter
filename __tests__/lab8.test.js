...
it('Clicking the "Add to Cart" button should change button text', async () => {
  ...
  let itemShadow = await prodItem.getProperty('shadowRoot');
  let elemButton = await itemShadow.$('button');
  // Once you have the button, you can click it and check the innerText property of the button.
  await elemButton.click();
  // Once you have the innerText property, use innerText.jsonValue() to get the text value of it
  let innerTxt = await elemButton.getProperty('innerText');
  let buttonTxt = await innerTxt.jsonValue();
  expect(buttonTxt).toBe("Remove from Cart");
  await elemButton.click(); //reset
}, 2500);
...
it('Checking number of items in cart on screen', async () => {
  ...
  let cartCountNum = await innerTxt.jsonValue();
